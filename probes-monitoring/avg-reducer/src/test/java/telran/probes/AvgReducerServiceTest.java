	...
	void MockSetUp() {
		...
		.then(new Answer<Optional<ProbesList>>() {
			...
			public Optional<ProbesList> answer(InvocationOnMock invocation) throws Throwable {
				Long sensorIdLong = invocation.getArgument(0);
				ProbesList probesList = redisMockMap.get(sensorIdLong);
				return probesList==null ? Optional.ofNullable(null)
						: Optional.of(probesList);
